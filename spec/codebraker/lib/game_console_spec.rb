...
RSpec.describe Lib::GameConsole do
  subject(:game_console) { described_class.new(player_name, difficulty) }
  ...
  describe '#run' do
    ...
    it 'command input' do
      allow(game_console).to receive(:input_guess).and_return({ type: :command, value: hint_command })
      expect(game_console).to receive(:hint_command)
      game_console.run
    end

    it 'guess input' do
      allow(game_console).to receive(:input_guess).and_return({ type: :input, value: guess })
  ...
  describe '#hint_command' do
    ...
    it 'give no hint message if hint empty' do
      ...
      expect(game_console).to receive(:output_no_hint_message)
      game_console.run
    end
  end
  ...
  describe '#show_result' do
    ...
    context 'when user win' do
      before do
        allow(game_console).to receive(:input_guess)
      end

      after { game_console.show_result(win_result) }

      it 'save result if answer yes' do
        allow(game_console).to receive(:input_approve_command).and_return('yes')
        expect(Lib::RatingConsole).to receive(:add_data)
      end

      it 'not save result' do
        allow(game_console).to receive(:input_approve_command).and_return('no')
