...
RSpec.describe GameConsole do
  ...
  describe '#show_result' do
    ...
    context 'when user win' do
      before do
        allow(game_console.input_helper).to receive(:guess)
      end
    ...
    context 'when user lost' do
      it 'lost' do
        ...
        allow(game_console.output_helper).to receive(:lost)
        allow(game_console.output_helper).to receive(:guess_result).with(lose_result[:code])
